...
class NetexecIngestor(BaseIngestor):
    name = "netexec"
    metadata = Metadata(
        description="Ingest NetExec databases and logs (.sam, .cached, .secrets, .db). Takes single files or folders in ~/.nxc. Not all tables are parsed yet. Feed host, service, password, ad_user, windows_user, etc.",
        version="0.0.1",
    )
