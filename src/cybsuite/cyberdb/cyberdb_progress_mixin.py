...
class CyberDBProgressMixin:
    ...
    def set_progress_total_portions(self, n: (int | None) = None, labels: (List[str | None] | None) = None) -> None:
        """
            Set the total number of progress portions (principle jobs).
            
            Args:
                n (int|None): The total number of progress portions.
                labels (list of str or None): list of labels for each portion.
            Raises:
                ValueError: If both n and labels are None.
                ValueError: If n is not a non-zero positive integer or not None.
                ValueError: If labels is not a list of (strings|None) or not None.
        """
        if n is None and labels is None:
            raise ValueError("Either n or labels must be provided")

        if n is not None and (not isinstance(n, int) or n <= 0):
            raise ValueError(
                "Portions number must be a non-zero positive integer."
            )

        if labels is not None and (not isinstance(labels, list) or not all(isinstance(x, (str, type(None))) for x in labels)):
            raise ValueError("Labels must be a list of strings|None, or None")

        if n is not None and labels is not None and n != len(labels):
            warning.warn("n must be equal to the length of labels. Taking the bigger one.")
            n = max(n, len(labels))
            labels += [None] * (n - len(labels))    # extend with None if labels is shorter
        elif n is None and labels is not None:
            n = len(labels)
        elif n is not None and labels is None:
            labels = [None] * n
    ...
    def set_progress_total_steps(self, n: (int | None) = None, labels: (List[str | None] | None) = None) -> None:
        """
            Set the total number of progress steps (sub-jobs) for the current portion.
            
            Args:
                n (int|None): The total number of progress steps.
                label (list of (str|None) or None): list of labels for each step.
            Raises:
        ...
        if n is not None and (not isinstance(n, int) or n <= 0):
            raise ValueError(
                "Steps number must be a non-zero positive integer."
            )
    ...
    def get_progress(self) -> dict:
        ...
        """
        portion_label = None if self._progress_portion_labels is None else self._progress_portion_labels[self._progress_current_portion] if self._progress_current_portion < self._progress_total_portions else None
        step_label = None if self._progress_steps_labels is None else self._progress_steps_labels[self._progress_current_step] if self._progress_current_step < self._progress_total_steps else None
