...
class Control:
    def __init__(
        self,
        scanner: "CyberDBPluginBaseMixin",
        name,
        *,
        details=None,
...
class CyberDBPluginBaseMixin:
    ...
    def are_controls_enabled(self, control_names: list[str] | str) -> bool:
        """Check if current run is enabled for given controls, by default all controls are enabled"""
        ...
        for control_name in control_names:
            if control_name not in self.controls:
                raise ValueError(
                    f"Control {control_name} must be declared in the plugin '{self.name}'.controls"
                )
