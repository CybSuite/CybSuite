...
interface ControlDefPageProps {
    isObservation?: boolean;
}

export default async function ControlDefPage({ isObservation }: ControlDefPageProps) {
    ...
    const [modelSchemaResponse, initialDataResponse, formSchemaResponse] = await Promise.all([
        serverApi.schema.getEntitySchema("control_definition_w_controls", cookieHeader, undefined, isObservation),
        serverApi.data.getEntityData("control_definition_w_controls", { skip: 0, limit: 10, isObservation }, cookieHeader),
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-3xl font-bold tracking-tight capitalize">
                        {isObservation ? "Observation Definition" : "Control Definition"} Management
                    </h1>
                    <p className="text-muted-foreground">
                        Manage and view {isObservation ? "observation definition" : "control definition"} data
                    </p>
                </div>
            </div>
            ...
            {modelSchemaResponse.error || initialDataResponse.error || formSchemaResponse.error ? (
                ...
                <>
                    ...
                    <ModelDataTable
                        ...
                        customDataFetchModelName="control_definition_w_controls"
                        customViewUrlPrefix={isObservation ? "/controls/observations/" : "/controls/controls/"}
                        customFieldOptionsEntityName="control_definition_w_controls"
                    />
                </>
            )}
        </div>
