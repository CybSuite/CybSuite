...
export interface IngestStatus {
  ...
  results: any | null;
  error: string | null;
  // Optional fields for detailed progress (may not be provided by backend)
  progress_bar?: number;
...
const getWebSocketUrl = () => {
  ...
  return `${wsUrl}/ws/ingest-status/`;
};

export const useIngestStatus = (): UseIngestStatusReturn => {
  ...
  const [logs, setLogs] = useState<IngestLogMessage[]>([]);
  ...
  const requestStatus = useCallback(async () => {
    // Don't make HTTP requests if we're not supposed to be polling (use ref as primary source of truth)
    if (!shouldPollRef.current) {
      return;
    }
    try {
      const response = await api.ingestors.getIngestStatus();
      if (response.data) {
      ...
      try {
        ...
        ws.onmessage = (event) => {
          try {
            ...
            if (data.type === "ingest_status" && data.status) {
              setStatus(data.status);
              setLastUpdate(new Date());
            } else if (data.type === "ingest_log" && data.data) {
              // Add unique ID and add to logs
              const logMessage: IngestLogMessage = {
                ...data.data,
  ...
  const forceWebSocketConnection = useCallback(() => {
    // Always clear polling, regardless of current state
    setUsePolling(false);
