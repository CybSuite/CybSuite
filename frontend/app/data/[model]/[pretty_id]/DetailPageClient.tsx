...
import ModelDataTable from '@/app/components/data/ModelDataTable';

interface DetailPageClientProps {
	initialSchema: EntitySchema;
	initialRecord: EntityRecord;
	model: string;
					...
					{displayItems.map((item, index) => {
						...
						return (
							<Badge
								...
								onClick={() => onRelationClick?.(typeInfo.referencedEntity || field.name, identifier)}
...
export default function DetailPageClient({
	...
	// Get the record's display representation
	...
	// Handle relation click - navigate to detail page
	const handleRelationClick = (entityType: string, identifier: string) => {
		if (identifier) {
			// Navigate to the detail page using pretty_id or fallback to numeric id
			window.open(`/data/${entityType}/${encodeURIComponent(identifier)}`, '_blank');
		}
	};

	// Get relation type for field (one-to-many, many-to-many, etc.)
	const getRelationType = (field: any): 'one-to-many' | 'many-to-many' | 'many-to-one' => {
		const typeInfo = parseFieldAnnotation(field);
		if (Array.isArray(record[field.name])) {
			return 'one-to-many'; // Current record has many of the related records
		}
		return 'many-to-one'; // Current record references one related record
	};
							...
							<Card key={fieldName} className={`transition-shadow hover:shadow-md ${getCardVariant()}`}>
										...
										<div className="flex items-center space-x-2">
											{typeInfo.isRelation && (
												<Button
													...
													onClick={() => {
														const entityType = typeInfo.referencedEntity || field.name.replace(/s$/, '');
														const tableId = `table-${entityType}`;
														document.getElementById(tableId)?.scrollIntoView({ behavior: 'smooth' });
													}}
													title={`Scroll to ${typeInfo.referencedEntity || field.name} table`}
												>
								...
								<CardContent className="pt-0">
									<div className="text-sm">
										{formatFieldValue(fieldValue, field, handleRelationClick)}
									</div>
								</CardContent>
							</Card>
						);
