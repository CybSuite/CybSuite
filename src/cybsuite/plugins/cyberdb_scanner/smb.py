...
class SmbScanner(BaseCyberDBScanner):
    name = "smb"
    metadata = Metadata(
        description="Scan for SMB services. Alert for SMB v1 and no-signing.",
        tags=["default"],
    )
    ...
    def do_run(self):
        ...
        self.set_progress_total_steps(len(services_smb))
        for service_smb in services_smb:
            self.set_progress_current_step_label(f"Scanning {str(service_smb)} record")
