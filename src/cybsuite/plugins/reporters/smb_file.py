...
class SmbFileReporter(BaseReporter):
    ...
    def run(self, output):
        ...
        highlighted = [(ext, stats) for ext, stats in all_extensions if stats.highlighted]
        non_highlighted = [(ext, stats) for ext, stats in all_extensions if not stats.highlighted][:50]
